package site.hanschen.common.statusbar;
...
public class StatusBarCompat {
    ...
    public static void setColor(Activity activity, @ColorInt int color, int alpha) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            if (count > 0 && decorView.getChildAt(count - 1) instanceof CompatView) {
                decorView.getChildAt(count - 1).setBackgroundColor(calculateStatusColor(color, alpha));
            } else {
                CompatView statusView = createStatusBarView(activity, color, alpha);
     ...
     * @param alpha 状态栏透明度
     */
    public static void setColorForSwipeBack(Activity activity, @ColorInt int color, int alpha) {
    ...
    public static void setColorForDrawerLayout(Activity activity, DrawerLayout drawerLayout, @ColorInt int color, int statusBarAlpha) {
        ...
        if (contentLayout.getChildCount() > 0 && contentLayout.getChildAt(0) instanceof CompatView) {
            contentLayout.getChildAt(0).setBackgroundColor(calculateStatusColor(color, statusBarAlpha));
        } else {
            CompatView statusBarView = createStatusBarView(activity, color);
     ...
     * @return 状态栏矩形条
     */
    private static CompatView createStatusBarView(Activity activity, @ColorInt int color) {
        // 绘制一个和状态栏一样高的矩形
        CompatView statusBarView = new CompatView(activity);
     ...
     * @return 状态栏矩形条
     */
    private static CompatView createStatusBarView(Context context, @ColorInt int color, int alpha) {
        CompatView statusBarView = new CompatView(context);
        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
     ...
     * @return 半透明 View
     */
    private static CompatView createTranslucentStatusBarView(Activity activity, int alpha) {
        // 绘制一个和状态栏一样高的矩形
        CompatView statusBarView = new CompatView(activity);
        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
                                                                         getStatusBarHeight(activity));
